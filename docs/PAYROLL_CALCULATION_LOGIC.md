...
| **天數計算** | 依據實際工作日與夜班數量。 | 日薪 = `Math.floor(月薪 × 12 ÷ 365)`（閏年 366）。月薪計糧以「實際工作日／正數手動天數」作為合資格錨點；星期日只要同一可計薪期間在其前後均有有效出勤便計入，即使中間平日缺勤後再復工。已滿三個月的法定假日仍必須位於由相鄰前後工作日夾住的連續區間。正常完整糧期上班保留月薪封頂及原有假日計算。 |
...
月薪的「工作日」只由 `payroll_work_logs` 或 `PayrollDailyCalc` 的正數手動日／夜班天數建立；**手動收入不是工作錨點**，不能單獨使休息日或假日合資格。零工作、零手動天數的月薪糧單不會產生基本薪金。零星工作的情況中，**星期日**只要於同一可計薪糧期已有此前出勤及後續復工，即可計薪；休息日後緊接的一個或多個平日缺勤不會取消該星期日。**合資格法定假日**仍須處於由相鄰前後兩個合資格工作日夾住的連續區間才計薪；糧期邊界不會推定糧期外有工作。完整糧期正常出勤則保留月薪封頂。
...
| 2026-09-21 | 月薪員工在星期日後暫時缺勤、其後復工時漏算休息日 | 月薪星期日改為只要同一糧期其前後均有有效出勤即計薪；法定假日的相鄰連續區間規則及日薪計算不變。 |
