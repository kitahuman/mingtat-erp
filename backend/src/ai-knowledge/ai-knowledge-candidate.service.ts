...
export class AiKnowledgeCandidateService {
  ...
  async createCandidateFromCorrection(
    params: KnowledgeCorrectionCandidateParams,
  ) {
    ...
    const title =
      params.title ??
      `欄位「${params.fieldName}」修正：${params.beforeValue ?? '空值'} → ${params.afterValue}`;
    ...
    const entry = await this.prisma.aiKnowledgeEntry.create({
      data: {
        ...
        knowledge_description:
          params.summary ?? '由人工修正 AI 識別結果自動生成的候選知識。',
  ...
  async checkPromotionThreshold(entryId: number) {
    ...
    if (!entry || entry.knowledge_status !== 'candidate')
      return { promoted: false };
    const policy = await this.prisma.aiKnowledgeModulePolicy.findUnique({
      where: { policy_module_code: entry.knowledge_module_code ?? '' },
    });
    const threshold = policy?.policy_review_threshold ?? 3;
    if (entry.knowledge_support_count < threshold)
      return {
        promoted: false,
        supportCount: entry.knowledge_support_count,
        threshold,
      };
    await this.prisma.aiKnowledgeEntry.update({
      where: { id: entryId },
      data: { knowledge_status: 'pending_review' },
    });
    return {
      promoted: true,
      supportCount: entry.knowledge_support_count,
  ...
  async recordUsageOutcome(
    knowledgeContextId: string,
    outcomes: KnowledgeUsageOutcome[],
  ) {
    const results: Array<{
      entryId: number;
      outcome: KnowledgeUsageOutcome['outcome'];
    }> = [];
    for (const outcome of outcomes) {
      ...
      await this.prisma.$transaction([
        ...
        this.prisma.aiKnowledgeEntry.update({
          where: { id: outcome.entryId },
          data: entryUpdate,
        }),
      ]);
      if (outcome.outcome === 'helpful')
        await this.checkPromotionThreshold(outcome.entryId);
  ...
  private async findSimilarKnowledge(
    ...
    return this.prisma.aiKnowledgeEntry.findFirst({
      where: {
        ...
        knowledge_status: {
          in: ['candidate', 'pending_review', 'approved', 'active'],
        },
        knowledge_applies_to_entity_type: params.entityType,
        knowledge_applies_to_entity_id: params.entityId,
        OR: searchTexts.map((keyword) => ({
  ...
  private async createEvidence(
    ...
    return this.prisma.aiKnowledgeEvidence.create({
      data: {
        ...
        evidence_summary:
          params.summary ??
          `人工確認 ${params.fieldName}：${params.beforeValue ?? '空值'} → ${params.afterValue}`,
        evidence_weight: 1,
