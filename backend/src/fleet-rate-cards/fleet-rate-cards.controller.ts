...
import { CreateFleetRateCardDto, UpdateFleetRateCardDto } from './dto/create-fleet-rate-card.dto';
...
export class FleetRateCardsController {
  ...
  @Post()
  create(@Body() dto: CreateFleetRateCardDto, @Request() req: any) {
    return this.service.create(dto, req.user?.id, req.headers['x-forwarded-for']?.toString().split(',')[0]?.trim() || req.ip || undefined);
  }

  @Put(':id')
  update(@Param('id') id: number, @Body() dto: UpdateFleetRateCardDto, @Request() req: any) {
