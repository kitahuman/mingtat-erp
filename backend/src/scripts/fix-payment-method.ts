...
function classify(referenceNo: string | null): string {
  if (!referenceNo || referenceNo.trim() === '') return '銀行轉帳';
  const ref = referenceNo.trim();
  // Pure 6-digit number → 支票
  if (/^\d{6}$/.test(ref)) return '支票';
  // Letters/symbols + digits combination → 支票
  if (/^[A-Za-z#]+\d+$/.test(ref) || /^[A-Za-z]+[#]?\d+$/.test(ref)) return '支票';
  // 7-digit number → 銀行轉帳
  if (/^\d{7}$/.test(ref)) return '銀行轉帳';
  // Fallback
  return '銀行轉帳';
}
...
async function main() {
  ...
  for (const record of records) {
    ...
    if (newMethod === '支票') chequeCount++;
    else bankTransferCount++;

    console.log(
      `  [${record.id}] ref="${record.reference_no ?? ''}" ${oldMethod} → ${newMethod}`,
    );
