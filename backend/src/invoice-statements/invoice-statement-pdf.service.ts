      ...
      .map((item: any, index: number) => {
        ...
        return `
          <tr>
            ...
            <td>${this.escapeHtml(invoice.invoice_no || '')}</td>
            <td>${this.formatDate(invoice.date)}</td>
            <td>${this.escapeHtml(invoice.invoice_title || '')}</td>
            <td class="center">${this.statusLabel(invoice.status)}</td>
            <td class="right">${this.formatMoney(invoice.total_amount)}</td>
            <td class="right">${this.formatMoney(invoice.paid_amount)}</td>
            <td class="right">${this.formatMoney(invoice.outstanding)}</td>
          </tr>`;
      })
      .join('');
...
<body>
  <div class="page">
      ...
      <table class="statement-table">
        <thead>
          <tr>
            ...
            <th>標題</th>
            <th class="center" style="width: 58px;">狀態</th>
    ...
    ${bankLines ? `<section class="bank"><div class="section-label">Payment / 付款資料</div><div class="bank-box">${bankLines}</div></section>` : ''}
