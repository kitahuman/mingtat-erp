...
export default function ContractManagementTabs({
  ...
  const loadContract = useCallback(() => {
    contractsApi.get(contractId).then(res => {
      ...
      setForm({ ...res.data });
      onContractLoaded?.(res.data);
      setLoading(false);
    }).catch(() => router.push(fallbackHref));
  }, [contractId, fallbackHref, onContractLoaded, router]);
  ...
  const tabs = showTabs ? contractManagementTabs : [];
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link href={backHref} className="hover:text-primary-600">{backLabel}</Link><span>/</span><span className="text-gray-900">{contract?.contract_no}</span>
      </div>
      )}
