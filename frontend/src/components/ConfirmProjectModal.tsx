...
export default function ConfirmProjectModal({
  ...
  const [clientId, setClientId] = useState<string>(
    target.client_id ? String(target.client_id) : '',
  );
  ...
  useEffect(() => {
    companiesApi
      .simple()
      .then((r: { data?: CompanyOption[] }) => setCompanies(r.data || []))
      .catch(() => setCompanies([]));
    partnersApi
      .simple()
      .then((r: { data?: PartnerOption[] }) =>
        setClients(
          (r.data || []).filter(
            (p) => p.partner_type === 'client' || p.partner_type === 'both',
          ),
        ),
      )
      .catch(() => setClients([]));
    contractsApi
      .simple()
      .then((r: { data?: ContractOption[] }) => setContracts(r.data || []))
      .catch(() => setContracts([]));
  }, []);
  ...
  const filteredContracts = contracts.filter(
    (c) => !clientId || String(c.client_id) === clientId,
  );
      ...
      >
        ...
        <div className="px-6 py-4 space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
            工程名稱：
            <span className="font-semibold">{target.project_name}</span>
            <p className="text-xs text-blue-700 mt-1">
              確認後系統會在「工程管理」中建立對應工程記錄，並自動更新所有關聯的日報記錄的
              project_id。
            </p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              內部公司 <span className="text-red-500">*</span>
            </label>
            <select
