...
import { triggerPageRefresh } from '@/hooks/usePageRefresh';
...
const navEntries: NavEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/reports/fixed-expenses', label: '固定支出統計', icon: '📋', pageKey: 'fixed-expense-report' },
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* Refresh Button */}
        <div className={`px-2 pt-2 pb-1 ${collapsed ? 'text-center' : ''}`}>
          <button
            onClick={handleRefreshClick}
            disabled={isRefreshing}
            title="重新整理"
            className={`
              flex items-center gap-2.5 px-4 py-2.5 rounded-lg transition-colors w-full text-sm
              ${isRefreshing 
