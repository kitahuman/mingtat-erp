...
// 支援兩種使用模式：
// 1. 自包含模式（最常用）：<CsvImportModal module="employees" onImportComplete={fetchData} />
// 2. 受控模式：<CsvImportModal module="employees" moduleName="員工" isOpen={open} onClose={close} onSuccess={refresh} />
interface Props {
  module: string;
  moduleName?: string;
  // 自包含模式
...
export default function CsvImportModal({ module, moduleName, onImportComplete, isOpen: isOpenProp, onClose: onCloseProp, onSuccess: onSuccessProp }: Props) {
  ...
  const isOpen = isControlled ? isOpenProp : selfOpen;
  ...
  return (
    <>
      ...
      {!isControlled && (
        <button
          onClick={handleOpen}
          className="px-3 py-1.5 text-sm bg-emerald-600 text-white rounded hover:bg-emerald-700 font-medium"
        >
          匯入 CSV
        </button>
      )}
                    ...
                    <table className="w-full text-xs">
                      <thead className="bg-gray-100 sticky top-0">
                        <tr>
                          ...
                          {preview.rows?.[0] && Object.keys(preview.rows[0]).filter(k => k !== '_rowNumber').map((key: string) => (
