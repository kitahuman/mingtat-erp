...
export default function SalaryConfigPage() {
  ...
  const defaultForm = {
    ...
    allowance_roller: 0,
    allowance_crane: 0,
    allowance_move_machine: 0,
    allowance_kwh_night: 0,
  ...
  const removeCustomAllowance = (idx: number) => {
    setForm({
      ...form,
      custom_allowances: form.custom_allowances.filter(
  ...
  const handleInlineSave = async (id: number, formData: any) => {
    ...
    if (formData.salary_type !== undefined)
      payload.salary_type = formData.salary_type;
    numFields.forEach((f) => {
      if (formData[f] !== undefined) payload[f] = Number(formData[f]) || 0;
    });
    dateFields.forEach((f) => {
      if (formData[f] !== undefined)
        payload[f] = toInputDate(formData[f]) || undefined;
    });
  ...
  const columns = [
    {
      ...
      render: (_: any, row: any) =>
        row.employee?.employee_is_temporary ? (
          <span className="text-gray-400 text-xs">臨時</span>
        ) : (
          row.employee?.emp_code || '-'
        ),
      filterRender: (_: any, row: any) => row.employee?.emp_code || '-',
    },
    {
      key: 'employee',
      label: '姓名',
    ...
    },
    {
      ...
      sortable: true,
      editable: false,
      render: (_: any, row: any) => row.employee?.role || '-',
      filterRender: (_: any, row: any) => row.employee?.role || '-',
    },
    {
      key: 'effective_date',
      label: '生效日期',
    ...
    },
    {
      ...
      editOptions: salaryTypeOptions,
      render: (v: any) => SALARY_TYPE_LABELS[v] || v,
      filterRender: (v: any) => SALARY_TYPE_LABELS[v] || v,
    },
    {
      key: 'base_salary',
      label: '日更底薪',
    ...
    },
    {
      ...
      render: (v: any, row: any) =>
        Number(v) > 0 ? (
          ...
          <span className="text-gray-400 text-xs">跟日更</span>
        ),
    },
    {
      key: 'allowance_night',
    ...
    },
    {
      ...
      sortable: true,
      editable: false,
      render: (v: any) => (v ? <span className="badge-blue">是</span> : '-'),
      filterRender: (v: any) => (v ? '是' : '否'),
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="card">
        <InlineEditDataTable
          exportFilename={
            activeTab === 'active' ? '在職員工薪酬' : '已離職員工薪酬'
          }
          ...
          onRowClick={(row) => openWorkspacePath(`/salary-config/${row.id}`)}
          loading={loading}
          sortBy={sortBy}
          sortOrder={sortOrder}
          onSort={(f, o) => {
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                {employees.map((emp: any) => (
                  <option key={emp.id} value={emp.id}>
                    {emp.emp_code} - {emp.name_zh || emp.name_en}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <input
                type="number"
                value={form.base_salary_night}
                onChange={(e) =>
                  setForm({ ...form, base_salary_night: e.target.value })
                }
                className="input-field"
                placeholder="0 = 跟日更底薪"
              />
          ...
          <div className="border-t pt-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={form.is_piece_rate}
                onChange={(e) =>
                  setForm({ ...form, is_piece_rate: e.target.checked })
                }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, change_amount: e.target.value })
                  }
                  className="input-field text-sm"
                />
              </div>
