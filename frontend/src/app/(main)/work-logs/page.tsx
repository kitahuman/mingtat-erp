...
import { STATUS_OPTIONS, getStatusLabel, STATUS_COLORS } from './constants';
...
const COLUMNS = [
  ...
  { key: 'quotation',     label: '報價單',   width: 'w-28' },
...
export default function WorkLogsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setQuotations(((qt.data?.data) || []).map((q: any) => ({ value: q.id, label: q.quotation_no, _raw: q })));
  ...
  }, [page, limit, filterPublisher, filterStatus, filterCompany, filterClient, filterQuotation, filterEmployee, filterEquipment, filterDateFrom, filterDateTo]);
  ...
  const resetFilters = () => {
    ...
    setFilterClient(null); setFilterQuotation(null); setFilterEmployee(null);
  ...
  const hasFilters = filterPublisher || filterStatus || filterCompany || filterClient ||
    filterQuotation || filterEmployee || filterEquipment || filterDateFrom || filterDateTo;
        ...
        <div className="flex items-center gap-2">
          {selected.size > 0 && (
            <>
              ...
              <button onClick={handleBulkConfirm} className="px-3 py-1.5 text-sm bg-green-600 text-white rounded hover:bg-green-700">
                批量確認
              </button>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 shrink-0">
        <div className="flex flex-wrap gap-2 items-end">
          <div className="flex flex-col gap-0.5">
            ...
            <div className="w-28">
              <SearchableSelect value={filterPublisher} onChange={v => { setFilterPublisher(v); setPage(1); }} options={users} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-xs text-gray-500">狀態</label>
            <div className="w-24">
              <SearchableSelect value={filterStatus} onChange={v => { setFilterStatus(v); setPage(1); }} options={STATUS_OPTIONS} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-xs text-gray-500">公司</label>
            <div className="w-28">
              <SearchableSelect value={filterCompany} onChange={v => { setFilterCompany(v); setPage(1); }} options={companyProfiles} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-xs text-gray-500">客戶公司</label>
            <div className="w-36">
              <SearchableSelect value={filterClient} onChange={v => { setFilterClient(v); setPage(1); }} options={clients} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-xs text-gray-500">報價單</label>
            <div className="w-32">
              <SearchableSelect value={filterQuotation} onChange={v => { setFilterQuotation(v); setPage(1); }} options={quotations} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
            <label className="text-xs text-gray-500">員工</label>
            <div className="w-28">
              <SearchableSelect value={filterEmployee} onChange={v => { setFilterEmployee(v); setPage(1); }} options={employees} placeholder="全部" />
            </div>
          </div>
          <div className="flex flex-col gap-0.5">
          ...
          {hasFilters && (
            <button onClick={resetFilters} className="px-3 py-1 text-xs text-gray-600 border border-gray-300 rounded hover:bg-gray-50 self-end">
              清除篩選
            </button>
          )}
        </div>
        ...
        <table className="border-collapse text-xs" style={{ minWidth: '2400px' }}>
          <thead className="sticky top-0 z-20 bg-gray-100 border-b-2 border-gray-300">
            <tr>
              ...
              {/* ID */}
              <th className="sticky left-8 z-30 bg-gray-100 px-2 py-2 border-r border-gray-300 w-12 text-left font-semibold text-gray-600">ID</th>
              {/* 發佈人 */}
              <th className="sticky left-20 z-30 bg-gray-100 px-2 py-2 border-r border-gray-300 w-20 text-left font-semibold text-gray-600">發佈人</th>
              {/* 狀態 */}
              <th className="sticky left-40 z-30 bg-gray-100 px-2 py-2 border-r border-gray-300 w-20 text-left font-semibold text-gray-600">狀態</th>
              {COLUMNS.slice(3).map(col => (
                <th key={col.key} className={`px-2 py-2 text-left font-semibold text-gray-600 whitespace-nowrap ${col.width}`}>
                  {col.label}
                </th>
              ))}
              {/* 操作 */}
              <th className="sticky right-0 z-30 bg-gray-100 px-2 py-2 border-l border-gray-300 w-28 text-left font-semibold text-gray-600">操作</th>
            </tr>
          </thead>
          <tbody>
            {/* New row at top */}
