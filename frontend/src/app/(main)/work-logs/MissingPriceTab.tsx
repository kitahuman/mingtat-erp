...
const emptyRateInputs = (): RateInputs => ({ rate: '', ot_rate: '', mid_shift_rate: '' });
...
export default function MissingPriceTab() {
  ...
  const getRateInputs = (key: string): RateInputs =>
    rateInputs[key] ?? emptyRateInputs();

  const setRateField = (key: string, field: keyof RateInputs, value: string) => {
    setRateInputs((prev) => ({
      ...prev,
  ...
  const handleAddRate = async (row: UnmatchedRow) => {
    ...
    if (!inputs.rate || isNaN(rate) || rate <= 0) {
      setToast({ message: '請輸入有效的費率（必填）', type: 'error' });
      return;
    }
        ...
        <table className="w-full text-sm border-collapse min-w-[1800px]">
          <thead className="sticky top-0 z-10 bg-gray-50">
            ...
            <tr className="border-b border-gray-200">
              ...
              <th className="px-2 py-2 text-left text-xs font-semibold text-gray-600 whitespace-nowrap">
                費率 <span className="text-red-500">*</span>
              </th>
              <th className="px-2 py-2 text-left text-xs font-semibold text-gray-600 whitespace-nowrap">
                OT費率
              </th>
          ...
          <tbody>
            {loading && data.length === 0 ? (
              <tr>
                <td colSpan={COLUMNS.length + 4} className="text-center py-12 text-gray-400">
                  載入中…
                </td>
              </tr>
            ) : data.length === 0 ? (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5">
                      <input
                        ...
                        value={inputs.rate}
                        onChange={(e) => setRateField(key, 'rate', e.target.value)}
                    ...
                    <td className="px-2 py-1.5">
                      <input
                        ...
                        onChange={(e) => setRateField(key, 'ot_rate', e.target.value)}
                        disabled={isSuccess || submitting[key]}
                      />
                    </td>
                    {/* 中直費率 */}
                    <td className="px-2 py-1.5">
                      <input
                        ...
                        placeholder="中直費率"
                        className="w-24 px-2 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-400"
                        value={inputs.mid_shift_rate}
                        onChange={(e) => setRateField(key, 'mid_shift_rate', e.target.value)}
                        disabled={isSuccess || submitting[key]}
                      />
                    </td>
                    {/* 操作 */}
