...
export default function ContractsPage() {
  ...
  useEffect(() => {
    partnersApi.simple().then((res) => {
      const clientPartners = (res.data || []).filter(
    ...
    <div>
              ...
              {mergeMode ? (
                <>
                  <button
                    ...
                    className="btn-primary bg-blue-600 border-blue-600"
                  >
                    確認合併 ({selectedIds.size})
                  </button>
                  <button onClick={toggleMergeMode} className="btn-secondary">
                    取消合併
                  </button>
                </>
              ) : (
      ...
      <div className="card">
        <InlineEditDataTable
          ...
          onRowClick={(row) =>
            mergeMode
              ? handleCheck(row.id, !selectedIds.has(row.id))
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <DateInput
                ...
                onChange={(v) => setForm({ ...form, sign_date: v })}
                className="input-field"
              />
            </div>
            <div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto border rounded p-2">
              {data
                ...
                .map((item) => (
                  <label
                    key={item.id}
                    className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded cursor-pointer border border-transparent has-[:checked]:border-blue-300 has-[:checked]:bg-blue-50"
                  >
                    <input
                      type="radio"
