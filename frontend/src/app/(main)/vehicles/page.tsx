...
export default function VehiclesPage() {
  ...
  const [createPlateForm, setCreatePlateForm] = useState({
    plate_number: '',
    owner_company_id: '',
    plate_owned_date: '',
    plate_expiry_date: '',
  ...
  const [plateTransferForm, setPlateTransferForm] = useState({
    to_company_id: '',
    transfer_date: '',
    notes: '',
  });
  ...
  const [plateColumnFilters, setPlateColumnFilters] = useState<
      ...
      .then((res) => {
        ...
        if (opts.length > 0) {
          ...
          if (syncDefault) {
            setForm((prev: any) => ({
              ...prev,
              machine_type: prev.machine_type || opts[0],
            }));
          }
        }
      })
  ...
  useRefetchOnFocus(() => {
    loadReferenceData().catch(() => {});
    loadVehicleTypeOptions(false);
  });

  const buildPlateColumnFilterParams = useCallback(
  ...
  const buildPlateListParams = useCallback(
  ...
  const handlePlateColumnFilterChange = (
    filters: Record<string, Set<string>>,
  ) => {
  ...
  const handleFetchPlateFilterOptions = useCallback(
    async (columnKey: string): Promise<string[]> => {
      const res = await vehiclePlatesApi.filterOptions(
        columnKey,
        buildPlateListParams({ page: 1, limit: 20 }),
      );
  ...
  const handlePlateExportFetchAll = useCallback(async () => {
    if (total === 0) return [];
    const res = await vehiclePlatesApi.list(
      buildPlateListParams({ page: 1, limit: total }),
    );
  ...
  const handleCreatePlate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || '新增車牌失敗');
    }
  };
  ...
  const handleRestore = async (vehicle: any) => {
    if (
      !confirm(
        `確定要復原車輛 ${vehicle.plate_number || vehicle.vehicle_original_plate || vehicle.id}？`,
      )
    )
      return;
  ...
  const exportScrappedCsv = () => {
    ...
    const rows = scrappedVehicles.map((v) => [
      ...
      v.vehicle_original_plate ||
        v.current_plate?.plate_number ||
        v.plate_number ||
        '',
  ...
  const handleAssignPlate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await vehiclePlatesApi.assign(selectedPlate.id, {
      ...
      alert(err.response?.data?.message || '套牌失敗');
    }
  };

  const openTransferPlateModal = (plate: any) => {
    setSelectedPlate(plate);
    setPlateTransferForm({
      to_company_id: '',
  ...
  const handleTransferPlate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await vehiclePlatesApi.transfer(selectedPlate.id, {
        from_company_id: selectedPlate.owner_company_id,
        to_company_id: Number(plateTransferForm.to_company_id),
        transfer_date: plateTransferForm.transfer_date,
        notes: plateTransferForm.notes,
      });
      setShowTransferPlateModal(false);
      await Promise.all([load(), loadReferenceData()]);
    } catch (err: any) {
      alert(err.response?.data?.message || '過戶失敗');
    }
  };
  ...
  const columns = [
    {
      ...
      render: (v: string, row: any) => (
        <span className="font-mono font-bold">
          {row.current_plate?.plate_number || v}
        </span>
      ),
    },
    {
      ...
      editOptions: vehicleTypes.map((t) => ({ value: t, label: t })),
    },
    {
      key: 'owner_company',
      label: '車主',
      ...
      render: (_: any, row: any) =>
        row.owner_company?.internal_prefix || row.owner_company?.name || '-',
  ...
  const renderCompany = (company: any) =>
    company?.internal_prefix
      ? `${company.internal_prefix} - ${company.name}`
      : company?.name || '-';
      ...
      : []),
    {
      ...
      render: (_: any, row: any) => (
        ...
        >
          ...
          <button
            onClick={() => openTransferPlateModal(row)}
            className="text-primary-600 hover:underline"
          >
            過戶
          </button>
    ...
    <div>
        ...
        <nav className="flex gap-6">
          {[
            ...
            <button
              ...
              className={`py-3 text-sm font-medium border-b-2 ${activeTab === key ? 'border-primary-600 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              {label}
            </button>
          ))}
      ...
      {activeTab === 'active' && (
        <div className="card">
          <InlineEditDataTable
            ...
            onRowClick={(row, event) => openWorkspacePath(`/vehicles/${row.id}`, undefined, event)}
            loading={loading}
            sortBy={sortBy}
            sortOrder={sortOrder}
            onSort={handleSort}
      ...
      {activeTab === 'scrapped' && (
        <div className="card">
          <div className="flex flex-wrap gap-2 mb-4 justify-between">
            ...
            <button onClick={exportScrappedCsv} className="btn-secondary">
              匯出 CSV
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b">
                  ...
                  <th className="px-3 py-2 text-left">型號</th>
                  <th className="px-3 py-2 text-left">底盤號碼</th>
                  <th className="px-3 py-2 text-left">原車牌</th>
                  <th className="px-3 py-2 text-left">劏車日期</th>
                  <th className="px-3 py-2 text-left">原屬公司</th>
                  <th className="px-3 py-2 text-right">操作</th>
                </tr>
              </thead>
              <tbody>
                {scrappedVehicles.map((v) => (
                  <tr
                    key={v.id}
                    className="border-b hover:bg-gray-50 cursor-pointer"
                    onClick={(event) =>
                      openWorkspacePath(`/vehicles/${v.id}`, undefined, event)
                    }
                    onMouseDown={(event) => {
                  ...
                  >
                    ...
                    <td className="px-3 py-2">{fmtDate(v.scrapped_at)}</td>
            ...
            {!loading && scrappedVehicles.length === 0 && (
      ...
      {activeTab === 'plates' && (
        <div className="card">
          ...
          <DataTable
            exportFilename={
              plateTab === 'in_use' ? '使用中車牌列表' : '閒置車牌列表'
            }
            ...
            onRowClick={(row, event) => openWorkspacePath(`/vehicles/plates/${row.id}`, undefined, event)}
            loading={loading}
            sortBy={plateSortBy}
            sortOrder={plateSortOrder}
          ...
          <div className="rounded-lg border p-3 bg-gray-50">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <label className="inline-flex items-center gap-2">
                <input
                  type="radio"
                  checked={form.plate_mode === 'existing'}
                  onChange={() =>
                    setForm({
                      ...form,
                      plate_mode: 'existing',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {form.plate_mode === 'new' ? (
              ...
              <div>
                ...
                <select
                  value={form.existing_plate_id}
                  onChange={(e) =>
                    setForm({ ...form, existing_plate_id: e.target.value })
                  }
                  className="input-field"
                  required
                >
                  ...
                  {idlePlates.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.plate_number} — {renderCompany(p.owner_company)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div>
              ...
              <DateInput
                ...
                onChange={(value) =>
                  setForm({ ...form, inspection_date: value })
                }
                className="input-field"
              />
            </div>
            <div>
              ...
              <DateInput
                ...
                onChange={(value) =>
                  setForm({ ...form, license_expiry: value })
                }
                className="input-field"
              />
            </div>
      ...
      <Modal
        isOpen={showCreatePlateModal}
        onClose={() => setShowCreatePlateModal(false)}
        title="新增車牌"
        size="lg"
      >
        <form onSubmit={handleCreatePlate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                value={createPlateForm.plate_number}
                onChange={(e) =>
                  setCreatePlateForm({
                    ...createPlateForm,
            ...
            <div>
              ...
              <select
                value={createPlateForm.owner_company_id}
                onChange={(e) =>
                  setCreatePlateForm({
                    ...createPlateForm,
                    owner_company_id: e.target.value,
                  })
                }
                className="input-field"
                required
            ...
            <div>
              ...
              <DateInput
                value={createPlateForm.plate_owned_date}
                onChange={(value) =>
                  setCreatePlateForm({
                    ...createPlateForm,
                    plate_owned_date: value,
                  })
                }
                className="input-field"
              />
            ...
            <div>
              ...
              <DateInput
                value={createPlateForm.plate_expiry_date}
                onChange={(value) =>
                  setCreatePlateForm({
                    ...createPlateForm,
                    plate_expiry_date: value,
                  })
                }
                className="input-field"
              />
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                value={createPlateForm.plate_notes}
                onChange={(e) =>
                  setCreatePlateForm({
                    ...createPlateForm,
                    plate_notes: e.target.value,
                  })
                }
                className="input-field"
                rows={3}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <button
              type="button"
              onClick={() => setShowCreatePlateModal(false)}
              className="btn-secondary"
            >
              取消
            </button>
      ...
      <Modal
        ...
        title={`套牌：${selectedPlate?.plate_number || ''}`}
      >
        <form onSubmit={handleAssignPlate} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              目標車輛 *
            </label>
          ...
          <div>
            ...
            <DateInput
              ...
              onChange={(value) =>
                setAssignForm({ ...assignForm, assigned_date: value })
              }
              className="input-field"
              required
            />
      ...
      <Modal
        isOpen={showTransferPlateModal}
        onClose={() => setShowTransferPlateModal(false)}
        title={`車牌過戶：${selectedPlate?.plate_number || ''}`}
      >
        <form onSubmit={handleTransferPlate} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              目前公司
            </label>
            <input
              value={renderCompany(selectedPlate?.owner_company)}
          ...
          <div>
            ...
            <select
              value={plateTransferForm.to_company_id}
              onChange={(e) =>
                setPlateTransferForm({
                  ...plateTransferForm,
            ...
            >
              <option value="">請選擇</option>
              {companies
                .filter((c) => c.id !== selectedPlate?.owner_company_id)
                .map((c) => (
                  <option key={c.id} value={c.id}>
                    {renderCompany(c)}
                  </option>
          ...
          <div>
            ...
            <DateInput
              value={plateTransferForm.transfer_date}
              onChange={(value) =>
                setPlateTransferForm({
                  ...plateTransferForm,
                  transfer_date: value,
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <button
              type="button"
              onClick={() => setShowTransferPlateModal(false)}
              className="btn-secondary"
            >
              取消
            </button>
