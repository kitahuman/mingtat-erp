    ...
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {editing ? (
            <>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">出生日期</label><DateInput value={toInputDate(form.date_of_birth)} onChange={val => setForm({...form, date_of_birth: val || ''})} className="input-field" /></div>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">入職日期</label><DateInput value={toInputDate(form.join_date)} onChange={val => setForm({...form, join_date: val || ''})} className="input-field" /></div>
              <div><label className="block text-sm font-medium text-gray-500 mb-1">離職日期</label><DateInput value={toInputDate(form.termination_date)} onChange={val => setForm({...form, termination_date: val || ''})} className="input-field" /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {editing ? (
            <>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">受僱日期(MPF)</label><DateInput value={toInputDate(form.mpf_employment_date)} onChange={val => setForm({...form, mpf_employment_date: val || ''})} className="input-field" /></div>
              <div><label className="block text-sm font-medium text-gray-500 mb-1">舊受僱日期</label><DateInput value={toInputDate(form.mpf_old_employment_date)} onChange={val => setForm({...form, mpf_old_employment_date: val || ''})} className="input-field" /></div>
            </>
          ) : (
            <>
            ...
            {STATIC_CERT_FIELDS.map(cert => (
              <div key={cert.noKey} className="border-b border-gray-100 pb-4 last:border-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div><label className="block text-xs font-medium text-gray-500 mb-1">到期日</label><DateInput value={toInputDate(form[cert.expiryKey])} onChange={val => setForm({...form, [cert.expiryKey]: val || ''})} className="input-field" /></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <DateInput
                          ...
                          onChange={val => setOtherCerts(prev => ({ ...prev, [ct.label]: { ...prev[ct.label], expiry_date: val || '', cert_no: prev[ct.label]?.cert_no || '' } }))}
                          className="input-field"
                        />
                      </div>
                    </div>
      ...
      <Modal isOpen={showSalaryModal} onClose={() => setShowSalaryModal(false)} title="新增薪資設定" size="lg">
        <form onSubmit={handleAddSalary} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">生效日期 *</label><DateInput value={salaryForm.effective_date} onChange={val => setSalaryForm({...salaryForm, effective_date: val || ''})} className="input-field" required /></div>
      ...
      <Modal isOpen={showTransferModal} onClose={() => setShowTransferModal(false)} title="員工調動">
        <form onSubmit={handleTransfer} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">調動日期 *</label><DateInput value={transferForm.transfer_date} onChange={val => setTransferForm({...transferForm, transfer_date: val || ''})} className="input-field" required /></div>
      ...
      <Modal isOpen={showTerminateModal} onClose={() => setShowTerminateModal(false)} title="員工離職">
        <form onSubmit={handleTerminate} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">離職日期 *</label><DateInput value={terminateForm.termination_date} onChange={val => setTerminateForm({...terminateForm, termination_date: val || ''})} className="input-field" required /></div>
