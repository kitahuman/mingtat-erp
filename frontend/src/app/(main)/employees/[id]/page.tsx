...
const roleOptions = [
  { value: 'driver', label: '司機' }, { value: 'operator', label: '機手' },
  { value: 'worker', label: '雜工' }, { value: 'admin', label: '管理' },
...
export default function EmployeeDetailPage() {
  ...
  // Filter certificates: in view mode, only show those that have data
  const visibleCerts = showAllCerts ? certificateFields : certificateFields.filter(c => emp?.[c.noKey] || emp?.[c.expiryKey]);
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-red-700 mt-1">
                <span>離職日期：{emp?.termination_date || '未記錄'}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {editing ? (
            <>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">出生日期</label><input type="date" value={form.date_of_birth || ''} onChange={e => setForm({...form, date_of_birth: e.target.value})} className="input-field" /></div>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">離職日期</label><input type="date" value={form.termination_date || ''} onChange={e => setForm({...form, termination_date: e.target.value})} className="input-field" /></div>
            ...
            <>
              ...
              <div><p className="text-sm text-gray-500">出生日期</p><p>{emp?.date_of_birth || '-'}{emp?.date_of_birth && <span className="text-gray-400 ml-2">({calcAge(emp.date_of_birth)} 歲)</span>}</p></div>
              ...
              {isTerminated && <div><p className="text-sm text-gray-500">離職日期</p><p className="text-red-600">{emp?.termination_date || '-'}</p></div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {editing ? (
            <>
              ...
              <div><label className="block text-sm font-medium text-gray-500 mb-1">受僱日期(MPF)</label><input type="date" value={form.mpf_employment_date || ''} onChange={e => setForm({...form, mpf_employment_date: e.target.value})} className="input-field" /></div>
              <div><label className="block text-sm font-medium text-gray-500 mb-1">舊受僱日期</label><input type="date" value={form.mpf_old_employment_date || ''} onChange={e => setForm({...form, mpf_old_employment_date: e.target.value})} className="input-field" /></div>
            </>
          ) : (
            <>
              ...
              <div><p className="text-sm text-gray-500">受僱日期(MPF)</p><p>{emp?.mpf_employment_date || '-'}</p></div>
              <div><p className="text-sm text-gray-500">舊受僱日期</p><p>{emp?.mpf_old_employment_date || '-'}</p></div>
            </>
          )}
        </div>
      </div>
            ...
            {certificateFields.map(cert => (
              <div key={cert.noKey} className="border-b border-gray-100 pb-4 last:border-0">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div><label className="block text-xs text-gray-500 mb-1">到期日</label><input type="date" value={form[cert.expiryKey] || ''} onChange={e => setForm({...form, [cert.expiryKey]: e.target.value})} className="input-field" /></div>
