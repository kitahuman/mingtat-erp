...
export default function SubconRateCardsPage() {
  ...
  const [allSubconFleet, setAllSubconFleet] = useState<
    { value: string; label: string; subcontractor_id?: number }[]
  >([]);
  ...
  useEffect(() => {
    load();
  }, [page, search, statusFilter, dayNightFilter, sortBy, sortOrder]);
  useEffect(() => {
    partnersApi.simple().then((res) => setPartners(res.data));
    companiesApi
      .list({ exclude_external: 'true' })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.source_quotation_id = form.source_quotation_id
        ? Number(form.source_quotation_id)
        : null;
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      editable: false,
      render: (_: any, row: any) => row.subcontractor?.name || '-',
      filterRender: (_: any, row: any) => row.subcontractor?.name || '-',
    },
    {
      key: 'plate_no',
      label: '車牌',
    ...
    },
    {
      ...
      editable: true,
      editType: 'select' as const,
      editOptions: [{ value: '', label: '-' }, ...tonnageOptions],
      render: (v: any) => v || '-',
    },
    {
      key: 'machine_type',
      ...
      editable: true,
      editType: 'select' as const,
      editOptions: [{ value: '', label: '-' }, ...vehicleTypeOptions],
      render: (v: any) => v || '-',
    },
    {
      key: 'origin',
      ...
      editable: true,
      editType: 'select' as const,
      editOptions: [{ value: '', label: '-' }, ...locationOptions],
      render: (v: any) => v || '-',
    },
    {
      key: 'destination',
    ...
    },
    {
      ...
      editType: 'select' as const,
      editOptions: [
        { value: false, label: '包油' },
        { value: true, label: '不包油' },
      ],
      render: (v: any) =>
        v ? (
          <span className="badge-red">不包油</span>
        ) : (
    ...
    <div>
      ...
      <div className="card">
        <InlineEditDataTable
          ...
          onRowClick={(row, event) => openWorkspacePath(`/subcon-rate-cards/${row.id}`, undefined, event)}
          loading={loading}
          sortBy={sortBy}
          sortOrder={sortOrder}
          onSort={(f, o) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  onChange={(v) =>
                    setForm({ ...form, subcon_id: v, plate_no: '' })
                  }
                  options={partners
                    .filter(
                      (p: any) =>
                        p.partner_type === 'subcontractor' ||
              ...
              <div>
                ...
                {form.subcon_id ? (
                  <Combobox
                    ...
                    options={allSubconFleet.filter(
                      (s) => s.subcontractor_id === Number(form.subcon_id),
                    )}
                    placeholder="選擇或輸入車牌"
                  />
                ) : (
              ...
              <div>
                ...
                <ClientContractCombobox
                  ...
                  onChange={(v) =>
                    setForm({ ...form, client_contract_no: v || '' })
                  }
                  placeholder="客戶合約"
                />
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.exclude_fuel}
                    onChange={(e) =>
                      setForm({ ...form, exclude_fuel: e.target.checked })
                    }
