...
// Create an axios instance that uses the main auth token for company-clock endpoints
const mainAuthClockApi = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_URL || '/api',
  headers: { 'Content-Type': 'application/json' },
  timeout: 120000,
});
mainAuthClockApi.interceptors.request.use((config) => {
...
const mainClockApi = {
  getEmployees: (params?: any) => mainAuthClockApi.get('/company-clock/employees', { params }),
...
export default function CompanyClockPage() {
  ...
  useEffect(() => {
    if (user) {
      mainClockApi.getCompanies().then((res) => {
  ...
  const loadEmployees = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const res = await mainClockApi.getEmployees(params);
  ...
  const loadTodayRecords = useCallback(async () => {
    try {
      const params: any = {};
      if (companyFilter) params.company_id = companyFilter;
      const res = await mainClockApi.getTodayAttendances(params);
  ...
  const handleSubmitClock = async () => {
    ...
    try {
      ...
      const res = await mainClockApi.clock({
