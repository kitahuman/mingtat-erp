...
export default function SubconFleetDriversPage() {
  ...
  useEffect(() => {
    load();
  }, [page, search, subconFilter, sortBy, sortOrder]);

  useEffect(() => {
    partnersApi.simple().then((res) => {
      // Only show subcontractor type partners as the fleet company options
      const subconOnly = (res.data || []).filter(
        (p: any) => p.partner_type === 'subcontractor',
      );
  ...
  const handleInlineSave = async (id: number, formData: any) => {
    ...
    if (payload.subcontractor_id)
      payload.subcontractor_id = Number(payload.subcontractor_id);
  ...
  const subcontractorOptions = partners.map((p) => ({
    value: p.id,
    label: p.name,
  }));
  ...
  const columns = [
    {
      ...
      editOptions: subcontractorOptions,
    ...
    },
    {
      ...
      render: (v: any) =>
        v === 'active' ? (
          ...
          <span className="badge-gray">停用</span>
        ),
    },
  ];
          ...
          filters={
            <div className="flex gap-2">
              <select
                value={subconFilter}
                onChange={(e) => {
                  setSubconFilter(e.target.value);
                  setPage(1);
                }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="input-field"
                placeholder="例如：文,阿文,文哥"
              />
              <p className="text-xs text-gray-400 mt-1">
                多個花名用逗號分隔，供 AI 辨認 WA order 司機用
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              ...
              <input
                type="text"
                value={form.yellow_cert_no}
                onChange={(e) =>
                  setForm({ ...form, yellow_cert_no: e.target.value })
                }
                className="input-field"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={form.red_cert_no}
                onChange={(e) =>
                  setForm({ ...form, red_cert_no: e.target.value })
                }
                className="input-field"
              />
            </div>
            <div className="flex gap-4 items-center h-full pt-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.has_d_cert}
                  onChange={(e) =>
                    setForm({ ...form, has_d_cert: e.target.checked })
                  }
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={form.is_cert_returned}
                  onChange={(e) =>
                    setForm({ ...form, is_cert_returned: e.target.checked })
                  }
                  className="rounded text-primary-600 focus:ring-primary-500 h-4 w-4"
                />
