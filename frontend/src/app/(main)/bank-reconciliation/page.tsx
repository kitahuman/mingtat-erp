      ...
      <div className="bg-white rounded-xl shadow-sm border">
        ...
        <div className="overflow-x-auto">
          ...
          <div className="bg-gray-50 border-b">
            <div
              className="grid min-w-[1120px] text-xs font-semibold text-gray-600 uppercase tracking-wider"
              style={{ gridTemplateColumns: reconciliationGridColumns }}
            >
              ...
              <div
                ...
                style={{
                  gridColumn: `span ${selectionMode ? 5 : 6} / span ${selectionMode ? 5 : 6}`,
                }}
              >
            ...
            {/* Sub-headers */}
            <div
              className="grid min-w-[1120px] text-[11px] font-medium text-gray-500 border-t border-gray-100"
              style={{ gridTemplateColumns: reconciliationGridColumns }}
            >
              {selectionMode && <div />}
              ...
              <div className="px-2 py-2 border-r border-gray-100 text-center">
                Ref No
              </div>
            ...
            {loading ? (
              ...
              transactions.map((tx: any) => {
                ...
                return (
                  <div
                    key={tx.id}
                    className={`group grid min-w-[1120px] text-sm hover:bg-gray-50 transition-colors ${tx.match_status === 'unmatched' ? 'bg-red-50/30' : ''} ${isSelected ? 'bg-blue-50' : ''}`}
                    style={{ gridTemplateColumns: reconciliationGridColumns }}
                  >
                    {/* Checkbox */}
                    {selectionMode && (
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        {tx.bank_txn_remark && (
                          <button
                            onClick={() => {
                              setRemarkTxId(tx.id);
                              setRemarkText(tx.bank_txn_remark || '');
                            }}
                            className="text-amber-500 hover:text-amber-700"
                            title={tx.bank_txn_remark}
                          >
                            <svg
                              className="w-3 h-3"
                              fill="currentColor"
                    ...
                    <div
                      className="px-2 py-2.5 border-r border-gray-50 whitespace-normal break-words leading-relaxed text-xs"
                    ...
                    <div className="px-2 py-2.5 border-r border-gray-50 text-xs text-gray-600">
                      {tx.match_status === 'matched' ? (
                        matchInfo.category
                      ) : tx.match_status === 'excluded' ? (
                        <span className="text-gray-400 italic">已排除</span>
                      ) : (
                        ''
                      )}
                    </div>
                    <div
                      className="px-2 py-2.5 border-r border-gray-50 truncate text-xs"
                    ...
                    <div className="px-2 py-2 flex items-center justify-center gap-1 relative">
                      {tx.match_status === 'unmatched' ? (
                        <div className="flex gap-1">
                          ...
                          <button
                            onClick={async () => {
                              await bankReconciliationApi.exclude(tx.id);
                              loadData();
                            }}
                          ...
                          >
                            ...
                            >
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth={2}
                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6"
                              />
                            </svg>
                          </button>
                        </div>
