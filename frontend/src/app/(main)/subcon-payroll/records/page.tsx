...
export default function SubconPayrollRecordsPage() {
  ...
  const handleDelete = async (id: number, name: string) => {
    if (
      !confirm(`確定要刪除 ${name} 的糧單嗎？此操作將同時刪除關聯的支出記錄。`)
    )
      return;
    try {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div>
            ...
            <SearchableSelect
              ...
              onChange={(v) => {
                setSelectedSubcon(v as number | null);
                setPage(1);
              }}
              placeholder="全部供應商"
            />
              ...
              data.map((row: any) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {formatMonth(row.subcon_payroll_month)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[row.subcon_payroll_status] || 'bg-gray-100 text-gray-800'}`}
                    >
                      {STATUS_LABELS[row.subcon_payroll_status] ||
                        row.subcon_payroll_status}
                    </span>
                  </td>
                  ...
                  >
                    <button
                      onClick={() =>
                        handleDelete(row.id, row.subcontractor?.name || '')
                      }
