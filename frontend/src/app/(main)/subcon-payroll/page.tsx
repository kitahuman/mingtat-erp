...
function GroupedView({
  ...
  const workTotal = groups.reduce(
    (s: number, g: any) => s + (Number(g.total_amount) || 0),
    0,
  );
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {groups.map((g: any, idx: number) => {
            const route = [g.start_location, g.end_location]
              ...
              .join(' → ');
            const hasPrice =
              g.price_match_status === 'matched' && g.matched_rate != null;
            return (
              <tr
                key={idx}
              ...
              >
                ...
                <td className="px-3 py-2 text-right font-mono">
                  {g.total_quantity > 0 ? g.total_quantity : g.count}車
                </td>
            ...
            .map((e, idx) => (
              <tr key={`extra-${idx}`} className="bg-blue-50">
                <td colSpan={7} className="px-3 py-2 text-blue-700 font-medium">
                  <span className="text-xs bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded mr-2">
                    其他
                  </span>
                  {e.name || '（未命名）'}
                </td>
        ...
        <tfoot className="border-t-2 border-gray-300">
          <tr className="bg-gray-50">
            <td
              colSpan={7}
              className="px-3 py-2 font-bold text-right text-gray-600"
            >
              工作記錄小計
            </td>
            <td className="px-3 py-2 text-right font-mono font-bold">
          ...
          {extraTotal > 0 && (
            <tr className="bg-gray-50">
              <td
                colSpan={7}
                className="px-3 py-2 font-bold text-right text-blue-600"
              >
                其他費用小計
              </td>
          ...
          <tr className="bg-primary-50">
            <td
              colSpan={7}
              className="px-3 py-2 font-bold text-right text-primary-700"
            >
              總計
            </td>
...
export default function SubconPayrollPage() {
  ...
  // ── Load reference data ──
  useEffect(() => {
    Promise.all([partnersApi.simple(), companiesApi.simple()])
      .then(([pt, cp]) => {
  ...
  const addExtraItem = () =>
    setExtraItems((prev) => [...prev, { name: '', amount: '' }]);
            ...
            <div key={idx} className="flex gap-3 items-center">
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  ...
                  value={item.amount}
                  onChange={(e) =>
                    updateExtraItem(idx, 'amount', e.target.value)
                  }
                  placeholder="金額"
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            {activeTab === 'grouped' && (
              <div className="p-4">
                <GroupedView
                  groups={groups}
                  extraItems={extraItems.filter((e) => e.name || e.amount)}
                />
              </div>
            )}
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {(result.work_logs || []).map((wl: any, i: number) => (
                      <tr
                        key={wl.id}
                        className={
                          wl._price_match_status === 'unmatched'
                            ? 'bg-red-50'
                            : 'hover:bg-gray-50'
                        }
                      >
                        ...
                        <td className="px-3 py-2">
                          {wl._driver?.name_zh || wl.employee?.name_zh || '-'}
                        </td>
                        ...
                        <td className="px-3 py-2 text-right">
                          {wl._matched_rate != null
                            ? `$${Number(wl._matched_rate).toFixed(0)}/${wl._matched_unit || '車'}`
                            : '-'}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {wl._line_amount
                            ? `$${Number(wl._line_amount).toFixed(0)}`
                            : '-'}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {wl._ot_line_amount
                            ? `$${Number(wl._ot_line_amount).toFixed(0)}`
                            : '-'}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {wl._mid_shift_line_amount
                            ? `$${Number(wl._mid_shift_line_amount).toFixed(0)}`
                            : '-'}
                        </td>
                        <td className="px-3 py-2 text-right font-medium">
                          {wl._total_amount
                            ? `$${Number(wl._total_amount).toFixed(0)}`
                            : '-'}
                        </td>
                        <td className="px-3 py-2">
                      ...
                      <tr>
                        ...
                        <td className="px-3 py-2 text-right">
                          ...
                          {(result.work_logs || [])
                            .reduce(
                              (s: number, w: any) =>
                                s + (Number(w._mid_shift_line_amount) || 0),
                              0,
                            )
                            .toLocaleString()}
                        </td>
                        <td className="px-3 py-2 text-right text-primary-600">
                          $
                          {(result.summary?.total_amount || 0).toLocaleString(
                            undefined,
                            { minimumFractionDigits: 2 },
                          )}
                        </td>
