...
export default function RentalRateCardDetailPage() {
  ...
  const loadData = () => {
    rateCardsApi.get(Number(params.id)).then(res => {
      ...
      setLoading(false);
    }).catch(() => openWorkspacePath('/rental-rate-cards'));
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
        <Link href="/rental-rate-cards" className="hover:text-primary-600">客戶價目表</Link><span>/</span><span className="text-gray-900">{record?.client?.name} - {record?.name || record?.service_type}</span>
      </div>
      ...
      <div className="card mb-6">
        ...
        {editing ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-500 mb-1">起點</label>
              <Combobox
                value={form.origin || ''}
            ...
            <div><label className="block text-sm font-medium text-gray-500 mb-1">終點</label>
              <Combobox
