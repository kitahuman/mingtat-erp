...
// ── Component ──────────────────────────────────────────────────────
export default function DailyReportForm({ reportId, copyFromId }: Props) {
  ...
  useEffect(() => {
    const targetId = reportId || copyFromId;
    if (!targetId) return;
    
    setLoading(true);
    employeePortalApi.getDailyReport(targetId).then(res => {
      const r = res.data;
      setForm({
      ...
      if (isEdit) {
        ...
        // Copy mode: keep today's date and clear signature
        setForm(f => ({
          ...f,
          report_date: new Date().toISOString().split('T')[0],
          signature: '',
